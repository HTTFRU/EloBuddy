...
namespace HTTF_TopLane_Series
{
    class CheckVersion 
    {
        ...
        {
            WebRequest Request_GitHubVer = WebRequest.Create("https://github.com/HTTFRU/EloBuddy/blob/master/CheckVersion/ToplineseriesVersion.txt");
            using (var Version_Response = (HttpWebResponse)Request_GitHubVer.GetResponse())
            {
                ...
                if (LocalVersion != NoticeList[1])
                {
                    ...
                    {
                        Main.M_NVer.AddLabel(NoticeList[n]);
                    }
                }
            }
        }
